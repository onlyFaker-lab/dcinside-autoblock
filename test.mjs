...
// ── [27] 퀵차단으로 중복 차단된 경우 ───────────────────────
// 파딱 질문(2026-09-13): 분탕이 여러 글을 쓰면 식별 코드를 일일이 확인하기
// 귀찮아서 퀵차단을 쓴다. 차단된 사람의 못 본 댓글을 신고받아 다시 퀵차단하면
// 디시가 기존 차단을 해제하고 새 차단을 건다. 그러면 목록에 이런 두 줄이 남는다.
//
//   1차: 08.01 31일 → 해제됨   (중복 차단 때문에 만료 전에 풀림)
//   2차: 09.01 31일 → 차단 중
//
// 1차만 보면 '예정보다 일찍 풀렸다' → 완장이 직접 푼 것으로 오판하고 영영 건너뛴다.
// 이게 '가장 최근 행만 본다'는 규칙이 있는 이유다.
{
  console.log("\n[27] 퀵차단 중복 차단");
  const 지금 = new Date("2026-09-13T18:00:00");
  const 판 = (rows) => analyzeCode(parseBlockList(table(rows)), "aaa1111", {}, 지금).status;
  const 일차 = row({ code: "aaa1111", date: "2026.08.01", time: "10:00:00", state: "해제됨" });
  const 이차차단중 = row({ code: "aaa1111", date: "2026.09.01", time: "10:00:00", state: "차단 중" });
  const 이차만료 = row({ code: "aaa1111", date: "2026.08.10", time: "10:00:00", state: "해제됨" });

  eq("2차가 차단 중이면 그냥 둔다", 판([이차차단중, 일차]), "active");
  eq("2차도 만료되면 재차단 후보", 판([이차만료, 일차]), "candidate");

  // 목록 정렬을 믿고 싶지 않다. 옛 행이 위로 와도 결과가 같아야 한다.
  // active 를 찾을 때 목록 전체에서 '차단 중'인 행을 찾기 때문에 순서에 안 흔들린다.
  eq("순서가 뒤집혀도 차단 중이면 안 건드린다", 판([일차, 이차차단중]), "active");
  eq("순서가 뒤집혀도 만료면 후보", 판([일차, 이차만료]), "candidate");

  // 퀵차단이 31일이 아닐 수도 있다. 그래도 차단 중이면 그 기간 만료까지 안 본다.
  for (const d of ["7일", "1일", "1시간"]) {
    const b = row({ code: "aaa1111", date: "2026.09.01", time: "10:00:00", state: "차단 중", duration: d });
    eq(`2차가 ${d}이어도 차단 중`, 판([b, 일차]), "active");
  }
